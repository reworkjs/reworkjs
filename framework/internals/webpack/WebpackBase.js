...
import { requireRawRoot, resolveProject } from '../../util/RequireUtil';
import frameworkConfig from '../../server/framework-config';
import { isDev, isTest } from '../../util/EnvUtil';
import { resolveFramework } from '../../util/RequireUtil';
...
export default class WebpackBase {
  ...
  getEntry(): string[] {
    ...
    const entry = [resolveFramework('common/kernel')];
    ...
    if (this.isDev) {
      entry.unshift(
        ...
        resolveFramework('internals/dev-preamble.js'),
      );
    }
  ...
  getBabelConfig() {
    ...
    if (this.isDev) {
      config.plugins.push('react-hmre');
    }
...
function templateContent() {
  ...
  if (appDiv.length === 0) {
    console.info('Could not find #app in html entry point, inserting it.');
